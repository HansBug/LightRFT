...
- 本仓库实验记录的默认规范入口是 `plan/EXPERIMENT_RECORD_GUIDE.md`。
- 当用户要求“写实验记录”“补实验分析”“整理某轮实验到 `plan/`”“更新 worklog / 调参记录 / 排障记录”时，先读取该文件，再开始写。
- 除非用户明确指定其它路径，实验记录默认落在 `plan/` 下。
- 文档开头必须先写 `总体目标`、`中间结论`、`后续计划` 三部分，不能直接从实验表格或日志开始。
- 每个实验条目必须写 `实验名称`、`实验设置`、`实验观察`、`实验分析` 四项，并补 `collect/eval` 与 `learn` 曲线截图；如果缺图，必须明确写出缺失原因和补充路径。
- 对 W&B / TensorBoard 里的重要指标，必须明确写出指标名称，并解释两个问题：这个指标是用来看什么的，当前这个数值或走势意味着什么。
- 记录时要同时覆盖表现好的实验和表现差的实验，默认要求 3 组及以上 seed；如果少于 3 组，必须写出原因和稳定性局限。
- 结论必须基于可核实证据。不要编造指标、截图、run 名称、时间线或结论；缺失信息要显式写成 `待补充`、`未核实` 或 `当前无法确认`。
- 实验设置中，优先补充分支、commit、脚本路径、数据路径、W&B/TensorBoard run 信息和计算资源，方便后续复现与排查。
